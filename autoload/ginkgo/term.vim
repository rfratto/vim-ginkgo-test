...
function! ginkgo#term#new(bang, cmd) abort
  return ginkgo#term#newmode(a:bang, a:cmd, g:go_term_mode)
endfunction

" new creates a new terminal with the given command and window mode.
function! ginkgo#term#newmode(bang, cmd, mode) abort
  let mode = a:mode
  if empty(mode)
...
function! s:on_exit(job_id, exit_status, event) dict abort
  ...
  let errors = ginkgo#tool#ParseErrors(job.stdout)
