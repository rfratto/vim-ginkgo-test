...
function! ginkgo#jobcontrol#Spawn(bang, desc, for, args) abort
...
function! s:on_exit(job_id, exit_status, event) dict abort
  ...
  if a:exit_status == 0
    ...
    if get(g:, 'go_echo_command_info', 1)
      call ginkgo#util#EchoSuccess("[" . self.status_type . "] SUCCESS")
    endif
  ...
  if get(g:, 'go_echo_command_info', 1)
    call ginkgo#util#EchoError("[" . self.status_type . "] FAILED")
  endif

  let errors = ginkgo#tool#ParseErrors(std_combined)
  ...
  if !len(errors)
    ...
    call ginkgo#util#EchoError(std_combined[0])
    return
  endif
